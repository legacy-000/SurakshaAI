            ...
            try:
                ...
                if user_scope and user_scope.get("district_id"):
                    units = db.execute_non_query(
                        f"SELECT UnitID FROM Unit WHERE DistrictID = {int(user_scope['district_id'])} LIMIT 1000"
                    )
                    station_ids = [
                    ...
                    if not station_ids:
                        return DashboardStatsDTO(district_count=31, station_count=0)
                    case_where.append(
                ...
                district_count = _scalar_count(db.execute_non_query("SELECT COUNT(ROWID) FROM District")) or 31
                station_count = _scalar_count(db.execute_non_query("SELECT COUNT(ROWID) FROM Unit")) or 312
