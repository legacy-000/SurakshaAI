...
export function NetworkGraph({
  ...
  // label the 12 biggest nodes outright, however many there are
  const labelCutoff = useMemo(() => {
    const sizes = nodes.map((n) => n.size || 9).sort((a, b) => b - a);
    return sizes.length <= 40 ? 0 : (sizes[11] ?? 0);
  }, [nodes]);
    ...
    <div ref={wrapRef} style={{ width: "100%" }}>
      ...
      >
        ...
        {nodes.map((n) => {
          ...
          // With ~90 nodes, labelling everything is unreadable and labelling
          // nothing leaves anonymous dots. Name the most-connected ones
          // permanently; the rest appear on hover.
          const named = active || n.size >= labelCutoff;
          return (
            <g key={n.id}
              ...
              {named && (
                <text x={0} y={r + 13} textAnchor="middle" fontSize={11}
                  fill="var(--text)" stroke="var(--bg)" strokeWidth={3}
                  paintOrder="stroke"
                  style={{ pointerEvents: "none", fontWeight: active ? 700 : 500,
                           opacity: hover == null || active ? 1 : 0.35 }}>
                  {n.label}
                </text>
              )}
