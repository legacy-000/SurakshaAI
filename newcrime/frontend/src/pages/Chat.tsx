...
export default function Chat() {
  ...
  const toggleMic = () => {
    ...
    // --- Pre-flight: secure context (https / localhost) ----
    if (!window.isSecureContext) {
      const msg = "Voice input requires a secure connection (HTTPS). This page is served over plain HTTP.";
      console.warn("[voice]", msg, location.href);
      setVoiceNote(msg);
      return;
    }
    ...
    const start = (code: string, retried = false) => {
      console.log("[voice] starting recognition, lang =", code);
      const r = new SR();
      r.lang = code;
      ...
      r.onend = () => { console.log("[voice] recognition ended"); setListening(false); };
      r.onerror = (e: any) => {
        const err = e?.error || "unknown";
        console.error("[voice] recognition error:", err, e);
        setListening(false);
        if (err === "language-not-supported" && !retried) {
          setVoiceNote("Kannada dictation unavailable — listening in English.");
          start("en-IN", true);
          return;
        }
        setVoiceNote(
          err === "not-allowed" || err === "service-not-allowed"
            ? "Microphone blocked. Allow mic access in your browser's site settings."
            : err === "no-speech" ? "Didn't catch that — try again."
            : err === "audio-capture" ? "No microphone found."
            : err === "network" ? "Speech service unreachable — check your internet connection."
      ...
      try {
        r.start();
        setListening(true);
        console.log("[voice] recognition started OK");
      } catch (ex) {
        console.error("[voice] start() threw:", ex);
        setListening(false);
        setVoiceNote("Could not start the microphone.");
      }
    };
    start(lang === "kn" ? "kn-IN" : "en-IN");
  };

  const speak = (text: string, l?: string) => {
    const want = l === "kn" ? "kn" : "en";
    // Setting utterance.lang alone is not enough: with no Kannada voice
    // installed the browser silently reads Kannada text with an English
    // voice. Pick a matching voice, and say so when there isn't one.
    const pick = () => {
      const voices = speechSynthesis.getVoices();
      const exact = voices.find((v) => v.lang?.toLowerCase().startsWith(want === "kn" ? "kn" : "en-in"));
      const loose = voices.find((v) => v.lang?.toLowerCase().startsWith(want));
      const voice = exact || loose;
      if (want === "kn" && !voice) {
        setVoiceNote("No Kannada voice installed on this device — install one in your OS speech settings.");
        return;
      }
      setVoiceNote("");
      const u = new SpeechSynthesisUtterance(text);
      u.lang = want === "kn" ? "kn-IN" : "en-IN";
      if (voice) u.voice = voice;
      speechSynthesis.cancel();
      speechSynthesis.speak(u);
    };
    // getVoices() is empty until the list loads on first use
        ...
        <div style={{ borderTop: "1px solid var(--border)", padding: 12 }}>
          ...
          {voiceNote && (
            <div style={{
              ...
              color: "#ffb020", display: "flex", alignItems: "center", gap: 8,
            }}>
              <span style={{ fontSize: 16 }}>⚠</span> {voiceNote}
            </div>
          )}
          {listening && (
            <div style={{
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="icon-btn" onClick={toggleMic}
              title={t("Voice input")} style={listening
