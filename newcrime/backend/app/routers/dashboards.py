...
def _in_scope(row, df):
    if df is None:
        return True
    d = row.get("district")
    return d in df if isinstance(df, list) else d == df
...
def _fd(rows, ctx):
    ...
    if df is None:
        return rows
    if isinstance(df, list):
        return [r for r in rows if r.get("district") in df]
...
def workspace(request: Request, db: Session = Depends(get_db)):
    ...
    if settings.use_catalyst:
        return _workspace_catalyst(ctx)
    df = ctx.district_filter()
...
# ── Catalyst dual-mode helpers ──────────────────────────────────────

def _workspace_catalyst(ctx):
    from ..catalyst_store import get_store
    store = get_store()
    df = ctx.district_filter()
    all_cases = store.query("SELECT * FROM cases")
    cases = _fd(all_cases, ctx)
    case_map = {str(c.get("ROWID")): c for c in all_cases}

    open_cases = sum(1 for c in cases if c.get("status") in ("Open", "Under Investigation"))
    needs_action = sum(1 for c in cases if c.get("status") == "Open")

    since = (datetime.utcnow() - timedelta(days=30)).isoformat()
    events = store.query("SELECT * FROM timeline_events WHERE event_type = 'Arrest'")
    arrests = sum(1 for e in events
                  if str(e.get("event_timestamp") or "") >= since
                  and _in_scope(case_map.get(str(e.get("case_id")), {}), df))

    alerts_filtered = _fd(store.query("SELECT * FROM alerts WHERE resolved = false"), ctx)
    my_alerts = len(alerts_filtered)

    role = ctx.role
    if role == "analyst":
        profiles = store.query("SELECT * FROM behavior_profiles")
        kpis = [
            {"label": "Detected Patterns", "value": store.count("crime_patterns")},
            {"label": "Active Forecasts", "value": store.count("predictions")},
            {"label": "High-risk Offenders",
             "value": sum(1 for p in profiles if p.get("risk_band") in ("High", "Critical")),
             "accent": "#ff4d5e"},
            {"label": "Open Alerts", "value": my_alerts, "accent": "#ffb020"},
        ]
    elif role in COMMAND_ROLES:
        ...
        profiles = store.query("SELECT * FROM behavior_profiles")
        kpis = [
            {"label": "Cases in Jurisdiction", "value": len(cases)},
            {"label": "Clearance Rate", "value": f"{round(solved/total*100,1)}%", "accent": "#24d18b"},
            {"label": "High-risk Offenders",
             "value": sum(1 for p in profiles if p.get("risk_band") in ("High", "Critical")),
             "accent": "#ff4d5e"},
    ...
    my_cases = sorted(cases, key=lambda c: c.get("occurrence_date") or "", reverse=True)[:8]

    all_events = store.query("SELECT * FROM timeline_events")
    ev_sorted = sorted(all_events, key=lambda e: e.get("event_timestamp") or "", reverse=True)
    stream = []
    for e in ev_sorted:
        c = case_map.get(str(e.get("case_id")))
        if c and _in_scope(c, df):
            stream.append({"title": e.get("event_title"), "type": e.get("event_type"),
                           "case": c.get("fir_number"), "district": c.get("district"),
    ...
    return {
        "officer": {"name": ctx.name, "role": role, "rank": ctx.caps["rank"],
                    "scope": ctx.scope,
                    "district": (", ".join(df) if isinstance(df, list) else df) if df else "All districts"},
        "kpis": kpis,
        "my_cases": [{"id": c.get("ROWID"), "fir_number": c.get("fir_number"), "title": c.get("title"),
                      "crime_type": c.get("crime_type"), "status": c.get("status"),
                      "severity": c.get("severity"), "district": c.get("district")} for c in my_cases],
        "stream": stream,
        "can_command": role in COMMAND_ROLES,
    }


def _command_catalyst(ctx):
    from ..catalyst_store import get_store
    ...
    m60 = (now - timedelta(days=60)).isoformat()
    this_month = sum(1 for c in cases if (c.get("occurrence_date") or "") >= m30)
    last_month = sum(1 for c in cases if m60 <= (c.get("occurrence_date") or "") < m30)

    events = store.query("SELECT * FROM timeline_events WHERE event_type = 'Arrest'")
    arrests = sum(1 for e in events
                  if str(e.get("event_timestamp") or "") >= m30
                  and _in_scope(case_map.get(str(e.get("case_id")), {}), df))

    pending_cases = by_status.get("Open", 0) + by_status.get("Under Investigation", 0)
    kpis = {
        "total_cases": len(cases), "open": pending_cases,
        "clearance_rate": round(solved / total * 100, 1),
        "firs_this_month": this_month,
        "firs_change": round((this_month - last_month) / last_month * 100, 1) if last_month else 0,
        "arrests_month": arrests,
        "by_status": [{"label": k, "value": v} for k, v in by_status.items()],
        "pending_cases": pending_cases,
        "chargesheet_rate": round(by_status.get("Chargesheeted", 0) / total * 100, 1),
        "conviction_rate": round(solved / total * 100, 1),
    ...
    for e in ev_sorted:
        ...
        if c and _in_scope(c, df):
            stream.append({"title": e.get("event_title"), "type": e.get("event_type"),
                           "case": c.get("fir_number"), "district": c.get("district"),
                           "severity": c.get("severity"), "time": e.get("event_timestamp")})
            if len(stream) >= 12:
                break

    # Alerts
    alerts_f = sorted(_fd(store.query("SELECT * FROM alerts WHERE resolved = false"), ctx),
                      key=lambda a: a.get("CREATEDTIME") or "", reverse=True)[:8]
    alerts = [{"id": a.get("ROWID"), "title": a.get("title"), "severity": a.get("severity"),
               "type": a.get("alert_type"), "district": a.get("district")} for a in alerts_f]

    # High-risk offenders
    profiles = store.query("SELECT * FROM behavior_profiles")
    accused_all = store.query("SELECT * FROM accused")
    accused_map = {str(a.get("ROWID")): a for a in accused_all}
    off_pairs = []
    for p in profiles:
        a = accused_map.get(str(p.get("accused_id")))
        if a and _in_scope(a, df):
            off_pairs.append((a, p))
    off_pairs.sort(key=lambda x: float(x[1].get("risk_score") or 0), reverse=True)
    offenders = [{"id": a.get("ROWID"),
                  "name": (a.get("full_name") if ctx.can_view_pii
                           else (a.get("full_name") or "?")[0] + "++++"),
                  "district": a.get("district"), "risk": round(float(p.get("risk_score") or 0)),
                  "band": p.get("risk_band")} for a, p in off_pairs[:8]]

    # Predictions
    preds_all = store.query("SELECT * FROM predictions")
    preds_f = preds_all
    if df:
        if isinstance(df, list):
            preds_f = [p for p in preds_all if p.get("target_area") in df]
        else:
            preds_f = [p for p in preds_all if p.get("target_area") == df]
    preds_f = sorted(preds_f, key=lambda p: float(p.get("probability") or 0), reverse=True)
    predictions = [{"area": p.get("target_area"), "crime": p.get("crime_type"),
                    "prob": p.get("probability"), "level": p.get("risk_level")}
                   for p in preds_f[:8]]

    # Breakdowns
    district_breakdown = [{"label": d, "value": v}
                          for d, v in Counter(c.get("district") for c in cases).most_common()]
    crime_type_breakdown = [{"label": ct, "value": v}
                            for ct, v in Counter(c.get("crime_type") for c in cases).most_common(10)]

    ai_insights = _ai_insights_catalyst(ctx, store, kpis, cases)
    ai_recs = _ai_recommendations(ctx, None, kpis, district_breakdown)
    neighbor_intel = _neighbor_intel_catalyst(ctx, store, all_cases, case_map)

    station_perf = [{"label": s, "value": v}
                    for s, v in Counter(c.get("station") for c in cases if c.get("station")).most_common()]

    # Investigation summary
    invs = store.query("SELECT * FROM investigations")
    inv_filtered = [i for i in invs
                    if _in_scope(case_map.get(str(i.get("case_id")), {}), df)]
    inv_by_status = Counter(i.get("status") for i in inv_filtered)
    progresses = [float(i.get("progress") or 0) for i in inv_filtered]
    investigation_summary = {
        "active": inv_by_status.get("Active", 0), "pending": inv_by_status.get("Pending", 0),
        "solved": inv_by_status.get("Solved", 0), "cold": inv_by_status.get("Cold", 0),
    ...
    if command_level in ("state", "range"):
        ...
        for c in cases:
            d = c.get("district")
            by_dist.setdefault(d, {"total": 0, "solved": 0})
            by_dist[d]["total"] += 1
            if c.get("status") in ("Chargesheeted", "Closed"):
                by_dist[d]["solved"] += 1
        district_ranking = sorted(
            [{"district": d, "total": v["total"], "solved": v["solved"],
              "clearance": round(v["solved"] / max(v["total"], 1) * 100, 1)}
             for d, v in by_dist.items()],
            key=lambda x: x["total"], reverse=True)[:15]

    # Station hotspots
    station_hotspots = None
    if command_level in ("district", "subdivision", "station", "range"):
        station_hotspots = _station_hotspots_catalyst(cases)

    forecast_analysis = _forecast_analysis_catalyst(ctx, store, cases)

    # Hotspot-aware AI recommendations
    if station_hotspots and station_hotspots.get("emerging"):
        for em in station_hotspots["emerging"][:2]:
            ai_recs.insert(0, {
                "action": f"Deploy additional patrols to {em['station']} — crime up {em['trend_pct']}% in 30 days",
                "priority": "Critical" if em["trend_pct"] > 50 else "High",
                "impact": f"Emerging hotspot with {em['cases_30d']} recent incidents.",
                "risk_score": min(95, em["heat_score"] + 10), "confidence": 0.88,
                "category": "Hotspot Response",
            })
    if forecast_analysis["forecasts"]:
        top_f = forecast_analysis["forecasts"][0]
        if top_f["probability"] >= 80:
            ai_recs.insert(1, {
                "action": f"Preemptive action for predicted {top_f['crime_type']} in {top_f['area']} — {top_f['probability']}% probability",
                "priority": "Critical",
                "impact": f"Forecast window: {top_f['window_start']} to {top_f['window_end']}.",
                "risk_score": top_f["probability"], "confidence": 0.92,
                "category": "Forecast Alert",
            })
    ai_recs = ai_recs[:10]

    return {
        "scope": ctx.scope, "command_level": command_level,
        "district": (", ".join(df) if isinstance(df, list) else df) if df else "All Karnataka",
        "subdivision": ctx.subdivision, "range_name": ctx.range_name,
        "kpis": kpis, "stream": stream, "alerts": alerts, "offenders": offenders,
        "predictions": predictions, "district_breakdown": district_breakdown,
        "crime_type_breakdown": crime_type_breakdown, "ai_insights": ai_insights,
        "ai_recommendations": ai_recs, "neighbor_intel": neighbor_intel,
        "station_performance": station_perf, "investigation_summary": investigation_summary,
        "district_ranking": district_ranking, "station_hotspots": station_hotspots,
        "forecast_analysis": forecast_analysis,
    }


def _ai_insights_catalyst(ctx, store, kpis, cases):
    from collections import Counter
    insights = []

    crime_dist = Counter(c.get("crime_type") for c in cases).most_common(5)
    if crime_dist:
        top = crime_dist[0]
        insights.append({"title": f"{top[0]} leads with {top[1]} cases",
                         "detail": f"{top[0]} is the most prevalent crime type in your jurisdiction.",
                         "severity": "High", "category": "Crime Analysis"})

    since_90 = (datetime.utcnow() - timedelta(days=90)).isoformat()
    recent = [c for c in cases if (c.get("occurrence_date") or "") >= since_90]
    hot = Counter(c.get("district") for c in recent).most_common(3)
    if hot:
        insights.append({"title": f"{hot[0][0]} requires immediate attention",
                         "detail": f"With {hot[0][1]} cases in 90 days, highest crime density.",
                         "severity": "Critical", "category": "Hotspot Analysis"})

    habitual = sum(1 for p in store.query("SELECT * FROM behavior_profiles")
                   if str(p.get("is_habitual")).lower() in ("true", "1"))
    if habitual:
        insights.append({"title": f"{habitual} habitual offenders active",
                         "detail": "Habitual offenders contribute disproportionately to crime rates.",
                         "severity": "High", "category": "Offender Intelligence"})

    pending_inv = len(store.query("SELECT * FROM investigations WHERE status = 'Pending'"))
    if pending_inv:
        insights.append({"title": f"{pending_inv} investigations pending",
                         "detail": "Stalled investigations impact clearance rate.",
                         "severity": "Medium", "category": "Investigation Bottleneck"})

    fin_count = sum(1 for c in cases if str(c.get("is_financial")).lower() in ("true", "1"))
    if fin_count > 10:
        insights.append({"title": f"Financial crime cluster -- {fin_count} cases",
                         "detail": "Rising financial crime suggests organized fraud networks.",
                         "severity": "High", "category": "Cybercrime Trend"})

    insights.append({"title": "Seasonal crime forecast: Festival season alert",
                     "detail": "Historical data shows 20-30% increase in property crimes.",
                     "severity": "Medium", "category": "Seasonal Forecast"})

    scope = ctx.scope
    if scope == "state":
        insights.extend([
            {"title": "Resource allocation imbalance detected",
             "detail": "Case-to-officer ratio varies significantly across districts.",
             "severity": "Medium", "category": "Resource Analysis"},
            {"title": "Organized crime network activity detected",
             "detail": "Cross-district networks identified. Coordinated operations recommended.",
             "severity": "Critical", "category": "Organized Crime"},
        ])
    elif scope in ("district", "range"):
        insights.append({"title": "Investigation workload unevenly distributed",
                         "detail": "Some officers carry 3x the average caseload.",
                         "severity": "Medium", "category": "Officer Workload"})
    elif scope in ("subdivision", "station"):
        insights.append({"title": "Community engagement opportunity identified",
                         "detail": "Repeat crime locations correlate with low community reporting.",
                         "severity": "Low", "category": "Community Policing"})
    return insights[:8]


def _neighbor_intel_catalyst(ctx, store, all_cases, case_map):
    from ..geo import get_neighbors
    from collections import Counter

    own_districts = ctx.districts_in_scope()
    if ctx.scope == "state":
        return _state_border_intel_catalyst(store, all_cases, case_map)

    neighbor_districts = list(set(
        d for own in own_districts for d in get_neighbors(own)
    ) - set(own_districts))
    if not neighbor_districts:
        return None

    since_90 = (datetime.utcnow() - timedelta(days=90)).isoformat()
    neighbor_cases = [c for c in all_cases
                      if c.get("district") in neighbor_districts
                      and (c.get("occurrence_date") or "") >= since_90]

    ca_rows = store.query("SELECT * FROM case_accused")
    own_accused = {str(r.get("accused_id")) for r in ca_rows
                   if case_map.get(str(r.get("case_id")), {}).get("district") in own_districts}
    neighbor_accused = {str(r.get("accused_id")) for r in ca_rows
                        if case_map.get(str(r.get("case_id")), {}).get("district") in neighbor_districts}
    shared_suspects = len(own_accused & neighbor_accused)

    by_district = {}
    for c in neighbor_cases:
        d = c.get("district")
        by_district.setdefault(d, {"district": d, "total": 0, "crimes": Counter()})
        by_district[d]["total"] += 1
        by_district[d]["crimes"][c.get("crime_type")] += 1

    neighbor_summary = sorted(
        [{"district": v["district"], "total": v["total"],
          "top_crimes": [{"crime": ct, "count": n} for ct, n in v["crimes"].most_common(3)]}
         for v in by_district.values()],
        key=lambda x: x["total"], reverse=True)[:6]

    cb_alerts = []
    if shared_suspects > 0:
        cb_alerts.append({"title": f"{shared_suspects} suspects operate across jurisdiction boundaries",
                          "detail": "Cross-district criminal movement detected.", "severity": "High"})
    for nd in neighbor_districts[:3]:
        nd_count = sum(1 for c in neighbor_cases if c.get("district") == nd)
        if nd_count > 15:
            cb_alerts.append({"title": f"High crime activity near {nd} border -- {nd_count} cases in 90 days",
                              "detail": f"Incidents in {nd} may spill over.", "severity": "Medium"})

    return {"neighbor_districts": neighbor_districts, "neighbor_summary": neighbor_summary,
            "shared_suspects": shared_suspects, "cross_border_alerts": cb_alerts[:4]}


def _state_border_intel_catalyst(store, all_cases, case_map):
    from ..geo import get_neighbors
    from collections import Counter
    ...
    accused_districts = {}
    for r in ca_rows:
        c = case_map.get(str(r.get("case_id")), {})
        if (c.get("occurrence_date") or "") >= since_90 and c.get("district"):
            accused_districts.setdefault(str(r.get("accused_id")), set()).add(c["district"])

    cross_border_count = {}
    for districts in accused_districts.values():
        if len(districts) > 1:
            for d in districts:
                cross_border_count[d] = cross_border_count.get(d, 0) + 1

    top_border = sorted(cross_border_count.items(), key=lambda x: x[1], reverse=True)[:6]
    shared_suspects = sum(1 for d in accused_districts.values() if len(d) > 1)

    recent_cases = [c for c in all_cases if (c.get("occurrence_date") or "") >= since_90]
    neighbor_summary = []
    for dist, cnt in top_border:
        dist_cases = [c for c in recent_cases if c.get("district") == dist]
        crimes = Counter(c.get("crime_type") for c in dist_cases).most_common(3)
        neighbor_summary.append({
            "district": dist, "total": len(dist_cases), "cross_border_suspects": cnt,
            "top_crimes": [{"crime": ct, "count": n} for ct, n in crimes],
        })

    alerts = []
    if shared_suspects > 5:
        alerts.append({"title": f"{shared_suspects} suspects across multiple districts statewide",
                       "detail": "Significant cross-district criminal movement.",
                       "severity": "Critical"})
    for dist, cnt in top_border[:3]:
        neighbors = get_neighbors(dist)
        if neighbors:
            alerts.append({"title": f"High cross-border activity near {dist}–{neighbors[0]} corridor",
                           "detail": f"{cnt} suspects in 90 days.", "severity": "High"})

    return {"neighbor_districts": [d for d, _ in top_border], "neighbor_summary": neighbor_summary,
            "shared_suspects": shared_suspects, "cross_border_alerts": alerts[:4]}


def _station_hotspots_catalyst(cases):
    from collections import Counter
    now = datetime.utcnow()
    m30 = (now - timedelta(days=30)).isoformat()
    m60 = (now - timedelta(days=60)).isoformat()

    recent = [c for c in cases if (c.get("occurrence_date") or "") >= m30 and c.get("station")]
    previous = [c for c in cases if m60 <= (c.get("occurrence_date") or "") < m30 and c.get("station")]

    by_station = {}
    for c in recent:
        s = c.get("station")
        by_station.setdefault(s, {"count": 0, "lats": [], "lons": [], "crimes": Counter(), "severity": Counter()})
        by_station[s]["count"] += 1
        if c.get("latitude"):
            by_station[s]["lats"].append(float(c["latitude"]))
        if c.get("longitude"):
            by_station[s]["lons"].append(float(c["longitude"]))
        by_station[s]["crimes"][c.get("crime_type")] += 1
        by_station[s]["severity"][c.get("severity")] += 1

    prev_map = Counter(c.get("station") for c in previous)
    max_cases = max((v["count"] for v in by_station.values()), default=1) or 1

    hotspots = []
    for station, v in by_station.items():
        count = v["count"]
        prev_count = prev_map.get(station, 0)
        trend_pct = round((count - prev_count) / max(prev_count, 1) * 100, 1)
        critical_high = v["severity"].get("Critical", 0) + v["severity"].get("High", 0)
        heat_score = round(count / max_cases * 70 + (critical_high / max(count, 1)) * 30)
        hotspots.append({
            "station": station, "cases_30d": count, "cases_prev_30d": prev_count,
            "trend_pct": trend_pct,
            "trend": "rising" if trend_pct > 10 else "falling" if trend_pct < -10 else "stable",
            "lat": round(sum(v["lats"]) / len(v["lats"]), 4) if v["lats"] else None,
            "lon": round(sum(v["lons"]) / len(v["lons"]), 4) if v["lons"] else None,
            "heat_score": min(heat_score, 100),
            "top_crimes": [{"crime": ct, "count": n} for ct, n in v["crimes"].most_common(3)],
    ...
    hotspots.sort(key=lambda h: h["heat_score"], reverse=True)
    emerging = [h for h in hotspots if h["trend"] == "rising" and h["cases_30d"] >= 2]
    return {"hotspots": hotspots[:10], "emerging": emerging[:5], "total_stations": len(hotspots)}


def _forecast_analysis_catalyst(ctx, store, cases):
    from collections import Counter
    df = ctx.district_filter()
    now = datetime.utcnow()
    ...
    preds_all = store.query("SELECT * FROM predictions")
    preds = [p for p in preds_all if (p.get("forecast_window_end") or "") >= now_iso]
    if df:
        if isinstance(df, list):
            preds = [p for p in preds if p.get("target_area") in df]
        else:
            preds = [p for p in preds if p.get("target_area") == df]
    preds.sort(key=lambda p: float(p.get("probability") or 0), reverse=True)
    ...
    for p in preds[:10]:
        ...
        forecasts.append({
            "area": p.get("target_area"), "crime_type": p.get("crime_type"),
            "probability": round(prob * 100) if prob <= 1 else round(prob),
            "risk_level": p.get("risk_level"),
            "window_start": (p.get("forecast_window_start") or "")[:10],
            "window_end": (p.get("forecast_window_end") or "")[:10],
            "factors": factors.split("; ") if factors else [],
        })

    m30 = (now - timedelta(days=30)).isoformat()
    m60 = (now - timedelta(days=60)).isoformat()
    cur_map = Counter(c.get("crime_type") for c in cases if (c.get("occurrence_date") or "") >= m30)
    prev_map = Counter(c.get("crime_type") for c in cases if m60 <= (c.get("occurrence_date") or "") < m30)

    crime_trends = []
    for ct in set(list(cur_map.keys()) + list(prev_map.keys())):
        cur, prev = cur_map.get(ct, 0), prev_map.get(ct, 0)
        change = round((cur - prev) / max(prev, 1) * 100, 1)
        crime_trends.append({"crime_type": ct, "current": cur, "previous": prev, "change_pct": change,
                            "trend": "rising" if change > 15 else "falling" if change < -15 else "stable"})
    crime_trends.sort(key=lambda x: x["change_pct"], reverse=True)
    ...
    return {"forecasts": forecasts, "crime_trends": crime_trends[:8],
            "risk_summary": by_risk, "total_forecasts": len(preds)}
