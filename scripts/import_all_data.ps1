Write-Host "Starting Suraksha AI Datastore Import..." -ForegroundColor Cyan
...
foreach ($table in $tables) {
    Write-Host "Importing table: $($table.name)..." -ForegroundColor Yellow
    catalyst ds:import $table.file --table $table.name
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Error importing $($table.name). Please check if the table exists in your Catalyst Datastore." -ForegroundColor Red
        exit $LASTEXITCODE
    }
    Write-Host "Successfully imported $($table.name)!" -ForegroundColor Green
}

Write-Host "All tables imported successfully!" -ForegroundColor Green
